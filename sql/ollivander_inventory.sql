-- Task
-- Write a query to print the id, age, coiuns_needed and power of the wands that Ron's interested in, sorted in order of descending power
-- If more than one wnad has the same power, sort the result in order of descending age
SELECT Wands.id, age, coins_needed, power
FROM Wands
INNER JOIN Wands_Property ON Wands.code=Wands_property.code
ORDER BY power DESC, age DESC;

-- The above was not the solution they wanted instead they wanted below
SELECT id, age, m.coins_needed, m.power FROM 
(SELECT code, power, MIN(coins_needed) AS coins_needed FROM Wands GROUP BY code, power) AS m
JOIN Wands AS w ON m.code = w.code AND m.power = w.power AND m.coins_needed = w.coins_needed
JOIN Wands_Property AS p ON m.code = p.code
WHERE p.is_evil = 0
ORDER BY m.power DESC, age DESC;